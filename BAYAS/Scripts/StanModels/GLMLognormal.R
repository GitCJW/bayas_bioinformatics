...
GLMLogNormalStanModel <- R6Class(classname = "GLMLogNormalStanModel",
                                inherit = BAYSISAbstractModel,
                                
                                public = list(
                                  
                                  id = 8,
                                  display_name = 'GLM - Log-normal distribution',
                                  description = 'The log-normal distribution is widely used for various kinds of data e.g. measures of size, time, blood pressure etc.,
                                  ...
                                  new_formula = function(response){
                                    ...
                                    res[[1]] <- FormulaElements$new("NoiseTerm",F,F,
                                                                    paste0("log(",response,")"), "~", tags$span(tags$span("Normal", class="formulaDistribution"),
                                                                                             "(",
                                                                                             tags$span(HTML("&mu;"), class="formulaParameter"),
                                                                                             ",",
                                                                                             tags$span(HTML("&sigma;"), class="formulaParameter"),
                                                                                             ")"),
                                                                    latex=toLatex(paste0("\\text{log}(",response,")"),"~","\\text{Normal}(\\mu, \\sigma)"),
                                                                    description="The logarithm of the response, that will be inferred with a normal (Gaussian) noise term.")
                                  ...
                                  getExplanation = function(response){
                                    ret <- GLMLogGaussianHelpDescription(response)
                                    return(ret)
                                  },
                                  ...
                                  get_content_for_stan_code = function(response, data, stanParameter){
                                    ...
                                    data[[response]] <- self$transformResponse(data[[response]])
                                  ...
                                  effects_description = function(type=c("slope","group")){
                                    if(type=="group"){
                                      return(paste0(
                                        "Understanding effects: For the log-normal model (identity link function) ",
                                        ...
                                        " A <b>group effect</b> is defined as the difference of the two (inner) group predictors."))
                                    }else if(type=="slope"){
                                      return(paste0(
                                  ...
                                  effects_description_latex = function(type=c("slope","group")){
                                    if(type=="group"){
                                      return(paste0(
                                        ...
                                        "a unit change of the predictor is identical to the change on the log(response). ",
                                        " In other words, the exponential of the effect is treated as a multiplier on the original scale.",
                                        " A group effect is defined as the difference of the two (inner) group predictors."))
                                    }else if(type=="slope"){
                                      return(paste0(
                                        "Understanding effects: For the normal model (identity link function) ",
                                        "a unit change of the predictor is identical to the change on the log(response). ",
                                  ...
                                  get_density = function(stanObject, data, mean=F){
                                    ...
                                    # epred <- exp(rnorm(4e3,5,1))
                                    # sigma <- rnorm(4e3, 1,0.1)
                                    ...
                                    if(mean){
                                      ...
                                      sigma <- c(post[,,"sigma"])
                                      pp <- prediction_dens_norm(log(epred), sigma)
                                      pp$x <- exp(pp$x)
                                      return(pp)
                                    }
                                  },
                                  ...
                                  make_predictions = function(stanObject, data, mean=F, draws=NULL){
                                    return(exp(super$make_predictions(stanObject, data, mean=mean, draws=draws)))
                                  },
                                  ...
                                  jacobianCorrection = function(x, response, ...){
                                    return(x-log(response))
                                  },
                                  
                                  plot_scale = function(x=F,y=F){
                                    if(x)return(scale_x_continuous(trans="log"))
                                    if(y)return(scale_y_continuous(trans="log"))
                                  }
